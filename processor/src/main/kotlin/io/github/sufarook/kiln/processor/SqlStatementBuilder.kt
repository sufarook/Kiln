...
object SqlStatementBuilder {
    ...
    fun createIndexes(meta: EntityMetadata): List<String> = meta.columns
        .filter { it.hasIndex }
        .map { col ->
    ...
    private fun pkWhereClause(meta: EntityMetadata): String = meta.primaryKeys.joinToString(" AND ") { "${it.columnName.q()} = ?" }
    ...
    fun delete(meta: EntityMetadata): String = "DELETE FROM ${meta.tableName.q()} WHERE ${pkWhereClause(meta)}"
