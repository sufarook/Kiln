...
2. **KSP isn't wired to the right source set.** In a KMP project, add the KSP dependency for the `commonMainMetadata` configuration:
   ...
   // build.gradle.kts
   dependencies {
       add("kspCommonMainMetadata", "com.farook.delightcrud:processor:1.0.0-alpha01")
   }
   ```
...
You applied the DelightCRUD plugin but KSP is missing. The plugin declares KSP as a required dependency — make sure you've added the KSP plugin to the plugins block:
...
### "DelightCRUD: @DbEntity must have exactly one @PrimaryKey property"
...
### "DelightCRUD: type 'X' is not supported"
...
Without `@Column(migrateFrom = "old_name")`, DelightCRUD treats the old column as dropped and the new column as added. All data in the old column defaults to the Kotlin default value.
...
SQLite requires a `DEFAULT` value when adding a `NOT NULL` column via `ALTER TABLE`. DelightCRUD uses the Kotlin default value for this.
...
### I have an existing SQLite database — can DelightCRUD manage it?
...
**Tables DelightCRUD doesn't know about** (no corresponding `@DbEntity`) are never touched.

---

### Can DelightCRUD work alongside raw SQL or Room?

Yes. DelightCRUD uses the `SqlDriver` directly — it's the same driver you'd pass to SQLDelight. You can call raw SQL through `driver.execute(…)` at any point. Just be aware that raw writes won't trigger DelightCRUD's `Query.Listener`, so reactive flows won't re-emit for those changes.
...
### Does DelightCRUD support transactions?
...
SQLite foreign keys are not enabled by default. DelightCRUD does not emit `PRAGMA foreign_keys = ON`. Handle referential integrity manually — see [Cross-table Loading](sample/cross-table.md) for the recommended pattern.

---

### Can I use DelightCRUD in a pure JVM project (no Android)?
